 ...
 * File: exp.h
 * -----------
 * This interface(接口) defines a class hierarchy for expressions,
 * which allows the client to represent and manipulate(操作) simple
 * binary(二元的，二进制的) expression trees.
 */
 ...
 */
    ...
    virtual bool DivideZero(EvalState &state) = 0;

    virtual bool isDefined(EvalState &state) = 0;
 ...
 */
    ...
    virtual bool DivideZero(EvalState &state);

    virtual bool isDefined(EvalState &state);
